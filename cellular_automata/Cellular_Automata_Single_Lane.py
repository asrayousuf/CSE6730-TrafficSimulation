...
def get_gap_o(i, lanes_data):
    ...
    try:
        gap = lanes_data[i:].index(1) - 1      
    except:
        gap = None #no car ahead in other lane      
    return gap


def get_gap_o_back(i, lanes_data):
    ...
    rev_o = lanes_data[:i+1]    #get list of cars behind in other lane
    rev_o.reverse()
...
def swap_data(data, pos):
    data[pos] = 1
    data[pos] = 0
    return(data)

def swap_data_raw(data, pos):
    data[pos] = data[pos]
    data[pos] = 0
    return(data)
...
def update_velocities(pos, lanes_data, velocity_data ,ttd_data, org_data):
    
    if get_gap(pos, lanes_data) != None:
            
        #accelerate if velocity less than max velocity
        if velocity_data[pos] < max_vel:
            velocity_data[pos] += 1
...
#             lanes_data, velocity_data, ttd_data, org_data = switch_lanes(pos,lane, lanes_data, velocity_data, ttd_data, org_data)
        if velocity_data[pos] > get_gap(pos,  lanes_data):
            velocity_data[pos] = get_gap(pos, lanes_data)
            
        # randomly slowing down     
        if velocity_data[pos] > 0 and random.random() < 0.3:
            velocity_data[pos] -= 1
...
def update(lanes_data, velocity_data, max_vel, ttd_data, TDd,t, org_data):
    ...
    for lane in [0,1]:
        occupied = get_occupied( lanes_data)
        for pos in reversed(occupied):
            ttd_data[pos] += 1
            lanes_data, velocity_data, ttd_data, org_data = update_velocities(pos, lanes_data, velocity_data, ttd_data, org_data)

    for lane in [0,1]:
        occ = get_occupied( lanes_data)  
        for i in reversed(occ):
            lights, = np.where((Lights > i) & (Lights < i + velocity_data[i]))
            if lights.size > 0:
                light = Lights[lights[0]]
                if NB_TR_signals(t,light) == 'red':
                    step = Lights[lights[0]] - i - 1
                else:
                    step = velocity_data[i]
            else:
                step = velocity_data[i]

            if step > 0:
                if (i + step < len(velocity_data)):
                    lanes_data[i + step] = 1
                    velocity_data[i + step] = step
                    org_data[i+step] = org_data[i]
                    org_data[i] = 0
                    ttd_data[i + step] = ttd_data[i]
                    ttd_data[i] = 0
...
def populate_lanes_from_10th(i, queue, lanes_data, org_data):
    ...
    if lanes_data[0] == 0 and queue[i] > 0:
        lanes_data[0] = 1 
        org_data[0] = 1
        queue[i] -= 1
...
#     i = 0 if pos == 30 else 1 if pos == 59 else None
    ...
    if lanes_data[76] == 0 and queue > 0:
        lanes_data[76] =1
        org_data[76] = 2
        queue -= 1
    return (queue, lanes_data, org_data)
...
def populate_by_right_at_light(pos, queue, lanes_data, org_data):
    ...
    * position 1 are entrances from 13th street
    ...
    if lanes_data[pos] == 0 and queue[i] > 0:
        lanes_data[pos] =1
        org_data[pos] = 2
        queue[i] -= 1
    return (queue, lanes_data, org_data)
...
#initialize model and run simulations
    ...
    for trial in range(100):
        ...
        cells, max_vel, ttd, TDd = initialize(151, 2)

        lanes = [0 for i in range(cells)]
        velocities = [0 for i in range(cells)]
        org = [0 for i in range(cells)]
        ...
        for key in TDd.keys():
            if Global.get(key) == None:
                Global[key] = TDd[key]
            else:
                Global[key] += TDd[key]

    summ = 0
    num = sum(list(Global.keys()))
    for key in Global.keys():
        summ += key*Global[key]
    mean = summ / num

    print('number of cars that traversed the corridor in 100 trials are %d \n'%(num))

    print('mean traversal time is %d \n'%(mean))

    print('The normalised distribution of traversals times is: ')
    for key in sorted(Global.keys()):
        n = Global[key]
        n = int(n/10.)
        print('%d:'%(key)+n*'*')
    ...
    print('\n')
    for key in sorted(Global.keys()):
        print('%s cars traversed the corridor in %d seconds'%(Global[key], key))
